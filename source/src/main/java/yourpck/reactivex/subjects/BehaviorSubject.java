...
import yourpck.reactivex.internal.util.AppendOnlyLinkedArrayList.NonThrowingPredicate;
 ...
 * <p>
 * Since a {@code BehaviorSubject} is an {@link yourpck.reactivex.Observable}, it does not support backpressure.
 * <p>
 * When this {@code BehaviorSubject} is terminated via {@link #onError(Throwable)} or {@link #onComplete()}, the
 * last observed item (if any) is cleared and late {@link yourpck.reactivex.Observer}s only receive
 ...
 * <dl>
 * <dt><b>Scheduler:</b></dt>
 * <dd>{@code BehaviorSubject} does not operate by default on a particular {@link yourpck.reactivex.Scheduler} and
 ...
 * {@link yourpck.reactivex.plugins.RxJavaPlugins#onError(Throwable)} (multiple times if multiple {@code Observer}s
 * cancel at once).
 ...
 * } </pre>
 *
 * @param <T> the type of item expected to be observed by the Subject
 */
