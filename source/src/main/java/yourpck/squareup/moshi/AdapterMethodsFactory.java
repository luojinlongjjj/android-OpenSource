...
import yourpck.squareup.moshi.internal.Util;
...
import static yourpck.squareup.moshi.internal.Util.jsonAnnotations;
...
final class AdapterMethodsFactory implements JsonAdapter.Factory {
    ...
    private static boolean parametersAreJsonAdapters(int offset, Type[] parameterTypes) {
        for (int i = offset, length = parameterTypes.length; i < length; i++) {
            ...
            if (((ParameterizedType) parameterTypes[i]).getRawType() != JsonAdapter.class)
                return false;
        }
        return true;
    }
        ...
        if (toAdapter == null || fromAdapter == null) {
            try {
                ...
                throw new IllegalArgumentException("No " + missingAnnotation + " adapter for "
                        + Util.typeAnnotatedWithAnnotations(type, annotations), e);
            }
        } else {
            delegate = null;
        }
    ...
    abstract static class AdapterMethod {
        ...
        public @Nullable
        Object fromJson(Moshi moshi, JsonReader reader)
                throws IOException, InvocationTargetException {
            throw new AssertionError();
        }
         ...
         */
        protected @Nullable
        Object invoke(@Nullable Object a1) throws InvocationTargetException {
